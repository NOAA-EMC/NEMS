...
!   type(ESMF_RouteHandle):: RHx2o      ! xgrid to ocn RH
  ...
  logical            :: generate_landmask = .false.   ! landmask flag
  integer            :: dbrc
...
!   call fld_list_add(fldsFrHyd, "water_surface_height_above_reference_datum"  , "cannot provide", "conservefrac")
  ...
  subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
    ...
    call ESMF_AttributeGet(gcomp, name="generate_landmask", value=value, defaultValue="false", &
                           convention="NUOPC", purpose="Instance", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    write(msgString,'(A,l6)')'MEDIATOR: Overwrite_Flag = ',overwrite_flag
    ...
    call ESMF_AttributeGet(gcomp, name="AoMedFlux", value=value,                          &
...
!             dataPtr_fieldAtm(i,j) = dataPtr_arrayAtm(i,j)
...
!-------------
! -- Create ESMFIO
          IOComp = ESMFIO_Create(gridAtmCoord, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,line=__LINE__, file=__FILE__)) return

! -- Write landmask files
          call ESMFIO_Write(IOComp, 'field_med_atm_a_land_mask.nc', (/fieldAtmMask/), filePath='./', rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!                      srcMaskValue=1,                                           &
    ...
    if (maptest .and. isPresentOcn) then
...
! -- Finalize ESMFIO  (commented out because calling the following maked the model to segfault)
...
!            endif
  ...
  subroutine DataInitialize(gcomp, rc)
    ...
    if (allDone) then
      ...
      !if (.not. coldstart) then
        ...
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      !endif
      write(0,*)' after Mediator_restart read'
  ...
  subroutine MedPhase_prep_atm(gcomp, rc)
    ...
    real(ESMF_KIND_R8), pointer :: ifrac_ae(:,:), ifrac_aer(:,:)  ! ice fraction on atm grid con2d map
...
!BL2017b
    character(ESMF_MAXSTR) ,pointer  :: fieldNameList(:)
    integer                          :: fieldCount, kdt
    integer*8                        :: sec8
    integer, parameter               :: deltim=450
    integer                          :: localPet
!BL2017b
...
!   call ESMF_ClockGet(clock,currtime=time,rc=rc)
    call ESMF_ClockGet(clock,currtime=time, startTime=startTime,rc=rc)
...
!  do we need to call the following four lines?
...
! use the nearest neighbor method
      ...
      do n = 1, fieldCount
        ...
        jl = lbound(dataPtr1,2) ; ju = ubound(dataPtr1,2)
        il = lbound(dataPtr1,1) ; iu = ubound(dataPtr1,1)


!---------------------------------------------------------------------------------------
!       if (trim(fldsFrOcn%shortname(n)) == 'sea_surface_temperature' .and. &
!       if (localpet == 171) then
!       if (trim(fieldNameList(n)) == 'sea_surface_temperature' .and. kdt >= 21 .and. kdt < 30) then

!         write(0,*)' field=',fieldNameList(n),' kdt=',kdt,' localpet=',localpet, &
!                   ' iliu=',il,iu,' jlju=',jl,ju
!         write(0,*)' dataptr1=',dataptr1(24,57), ' dataptr2=',dataptr2(24,57)
!         write(0,*)' sample dataptr1=',dataptr1((il+iu)/2,(jl+ju)/2), &
!                          ' dataptr2=',dataptr2((il+iu)/2,(jl+ju)/2)
!         write(1200+localpet,*)' kdt=',kdt
!         do j=jl,ju
!           write(1200+localpet,*)' dataPtr1=',dataPtr1(il:iu,j),' j=',j, &
!                                 ' name=',trim(fieldNameList(n))
!           write(1200+localpet,*)' dataPtr2=',dataPtr2(il:iu,j),' j=',j
!         enddo
!         if (localpet == 176) then
!           write(0,*)' dataPtr1=',dataPtr1(169,249), '
!           name=',trim(fldsFrIce%shortname(n)),' kdt=',kdt,' il=',il
!           write(0,*)' dataPtr2=',dataPtr2(169,249),' n=',n,' jlju=',jl,ju, &
!                     ' iliu=',il,iu
!         endif 
!         endif 
!---------------------------------------------------------------------------------------
...
!$omp parallel do default(shared) private(i,j)
        do j=jl, ju
          do i=il, iu
            if (abs(dataPtr1(i,j)) <= epsln .and. abs(dataPtr2(i,j)) > epsln) then
...
!     if (localpet == 276) write(0,*)' in medphase_prep_atm in i2a_active'
...
!$omp parallel do default(shared) private(i,j)
       ...
       !--- conservative 2nd order frac
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_con2f, rc=rc)) then
          ...
          allocate(ifrac_agr(lbound(dataptr2,1):ubound(dataptr2,1),lbound(dataptr2,2):ubound(dataptr2,2)))
          allocate(ifrac_ag (lbound(dataptr2,1):ubound(dataptr2,1),lbound(dataptr2,2):ubound(dataptr2,2)))
...
!$omp parallel do default(shared) private(i,j)
        ...
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_con2d, rc=rc)) then
          ...
          allocate(ifrac_aer(lbound(dataptr2,1):ubound(dataptr2,1),lbound(dataptr2,2):ubound(dataptr2,2)))
          allocate(ifrac_ae (lbound(dataptr2,1):ubound(dataptr2,1),lbound(dataptr2,2):ubound(dataptr2,2)))
...
!$omp parallel do default(shared) private(i,j)
              do j=jl,ju
                do i=il,iu
                  if (abs(dataPtr3(i,j)) <= epsln .and. abs(dataPtr4(i,j)) > epsln) then
...
!         if (trim(fldsFrIce%shortname(n)) == 'sea_ice_surface_temperature' .and. kdt >= 24 .and. localpet == 276) then
!             write(0,*)' dataPtr3=',dataPtr3(il+29,59),' name=',trim(fldsFrIce%shortname(n)),' kdt=',kdt,' il=',il
!           do j=jl,ju
!             write(1200+localpet,*)' dataPtr3=',dataPtr3(il:iu,j),' j=',j,' name=',trim(fldsFrIce%shortname(n)),' kdt=',kdt
!           enddo
!           write(0,*)' dataPtr3=',dataPtr3(222,315),' n=',n,' jlju=',jl,ju, &
!            ' iliu=',il,iu,' mapping=',fldsFrIce%mapping(n)
!         endif
        enddo
...
!$omp parallel do default(shared) private(i,j)
        ...
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_consf, rc=rc)) deallocate(ifrac_af, ifrac_afr)
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_consd, rc=rc)) deallocate(ifrac_ad, ifrac_adr)
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_con2f, rc=rc)) deallocate(ifrac_ag, ifrac_agr)
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_con2d, rc=rc)) deallocate(ifrac_ae, ifrac_aer)
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_bilnr, rc=rc)) deallocate(ifrac_ab, ifrac_abr)
        if (ESMF_RouteHandleIsCreated(is_local%wrap%RH_i2a_patch, rc=rc)) deallocate(ifrac_ap, ifrac_apr)
...
!     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    call fieldBundle_accum(is_local%wrap%FBaccumAtmOcn, is_local%wrap%FBAtmOcn_o, rc=rc)             ! Do we need this? Moorthi
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!$omp parallel do default(shared) private(i,j)
      do j=lbound(zbot,2),ubound(zbot,2)
        do i=lbound(zbot,1),ubound(zbot,1)
          if(tbot(i,j) <= epsln .and. tbot2(i,j) > epsln ) then
            zbot(i,j) = zbot2(i,j)
            tbot(i,j) = tbot2(i,j)
            ubot(i,j) = ubot2(i,j)
            vbot(i,j) = vbot2(i,j)
...
!   logical                     :: checkOK, checkOK1, checkOK2
  ...
  subroutine MedPhase_prep_ocn(gcomp, rc)
    ...
    type(InternalState)         :: is_local
    type(ESMF_VM)               :: vm
    integer                     :: i,j,n, il, iu, jl, ju, localPet
    character(ESMF_MAXSTR)      :: fieldname1(10),fieldname2(10),fieldname3(10)
    real(ESMF_KIND_R8), pointer :: dataPtr1(:,:),dataPtr2(:,:),dataPtr3(:,:)
    real(ESMF_KIND_R8), pointer :: atmwgt(:,:),icewgt(:,:),customwgt(:,:)
...
!$omp parallel do default(shared) private(i,j)
        do j=lbound(tmp_n1,2),ubound(tmp_n1,2)
          do i=lbound(tmp_n1,1),ubound(tmp_n1,1)
            if (abs(tmp_n1(i,j)) <= epsln .and. abs(tmp_n2(i,j)) > epsln) then
...
! hycom uses latent heat flux
        ...
        call fieldBundle_FieldMerge(is_local%wrap%FBforOcn     , 'mean_sensi_heat_flx'             , 1,      &
                                    is_local%wrap%FBAccumAtmOcn, 'mean_sensi_heat_flx_atm_into_ocn', atmwgt, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!                                  is_local%wrap%FBAccumAtmOcn, 'mean_up_lw_flx_ocn', atmwgt, rc=rc)
  !    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    !-------------
      ...
      call fieldBundle_FieldMerge(is_local%wrap%FBforOcn, 'mean_prec_rate', 1,       &
                                  is_local%wrap%FBAtm_o , 'mean_prec_rate', atmwgt,  rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    !-------------
      ...
      call fieldBundle_FieldMerge(is_local%wrap%FBforOcn, 'net_heat_flx_to_ocn', 1,      &
                                  is_local%wrap%FBIce_o , 'net_heat_flx_to_ocn', icewgt, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return


      call fieldBundle_FieldMerge(is_local%wrap%FBforOcn, 'mean_fresh_water_to_ocean_rate', 1,      &
      ...
      call fieldBundle_FieldMerge(is_local%wrap%FBforOcn, 'mean_salt_rate', 1,     &
                                  is_local%wrap%FBIce_o , 'mean_salt_rate', icewgt, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!
    ...
    call FieldBundle_RWFields_tiles(mode, fname, is_local%wrap%FBaccumAtm, rc=rc)
!   call FieldBundle_RWFields_tiles(mode,fname,is_local%wrap%FBaccumAtm,read_rest_FBaccumAtm,rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
#else
    fname = trim(bfname)//'_FBaccumAtm_restart.nc'
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumAtm, rc=rc)
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBaccumAtm,read_rest_FBaccumAtm,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumOcn, rc=rc)
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBaccumOcn,read_rest_FBaccumOcn,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumIce, rc=rc)
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBaccumIce,read_rest_FBaccumIce,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumLnd, rc=rc)
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBaccumLnd,read_rest_FBaccumLnd,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumHyd, rc)
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBaccumHyd,read_rest_FBaccumHyd,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBaccumAtmOcn, rc=rc)
...
!   call FieldBundle_RWFields(mode,fname,is_local%wrap%FBHyd_h,read_rest_FBHyd_h,rc=rc)
    ...
    call FieldBundle_RWFields(mode, fname, is_local%wrap%FBAtmOcn_o, rc=rc)
...
!       read_rest_FBaccumAtmOcn = .false.
  ...
  subroutine FieldBundle_RWFields(mode,fname,FB,rc)
...
#ifdef FV3_CPLD
  subroutine FieldBundle_RWFields_tiles(mode,fname,FB,rc)
...
! -- Finalize ESMFIO
  ...
  subroutine Compute_RHs(FBsrc, FBdst, bilnrmap, consfmap, consdmap, con2fmap, con2dmap, &
    ...
    logical :: do_con2f, do_con2d, do_nrnbr
    ...
    if (     present(nrnbrmap)) do_nrnbr = .true.
...
!BL2017
  ...
  subroutine Fieldbundle_Regrid2(fldlist, FBin, FBout, nrnbrmap,string, rc)
    type(fld_list_type)    :: fldlist
    type(ESMF_FieldBundle) :: FBin
    type(ESMF_FieldBundle) :: FBout
    type(ESMF_Routehandle),optional :: nrnbrmap
    ...
    oknearest = .false.
    if (present(nrnbrmap)) then
      if (ESMF_RouteHandleIsCreated(nrnbrmap, rc=rc)) oknearest = .true.
    endif
...
!-----------------------------------------------------------------------------
  ...
  subroutine Grid_Write(grid, string, rc)
    ...
    type(ESMF_Field)            :: field(1), fieldR(1)
    type(ESMF_GridComp)         :: IOComp
    ...
    if (isPresent) then
      ...
      field(1) = ESMF_FieldCreate(grid, array, staggerloc=ESMF_STAGGERLOC_CENTER, name="lon_center", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!     call ESMF_ArrayWrite(array, trim(string)//"_grid_coord1.nc", rc=rc)
      if (multitile) then
        ...
        call ESMF_FieldWrite(field(1), trim(string)//"_grid_coord1.nc", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      endif
      call ESMF_FieldDestroy(field(1),rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      ...
      field(1) = ESMF_FieldCreate(grid, array, staggerloc=ESMF_STAGGERLOC_CENTER, name="lat_center", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!     call ESMF_ArrayWrite(array, trim(string)//"_grid_coord2.nc", rc=rc)
      if (multitile) then
        ...
        call ESMF_FieldWrite(field(1), trim(string)//"_grid_coord2.nc", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      endif
      call ESMF_FieldDestroy(field(1),rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    if (isPresent) then
      ...
      if (.not. ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) then
        ...
        field(1) = ESMF_FieldCreate(grid, array, staggerloc=ESMF_STAGGERLOC_CORNER, name="lat_corner", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!       call ESMF_ArrayWrite(array, trim(string)//"_grid_corner1.nc", rc=rc)
        if (multitile) then
          ...
          call ESMF_FieldWrite(field(1), trim(string)//"_grid_corner1.nc", rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        endif
        call ESMF_FieldDestroy(field(1),rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!       call ESMF_ArrayWrite(array, trim(string)//"_grid_corner2.nc", rc=rc)
        if (multitile) then
          ...
          call ESMF_FieldWrite(field(1), trim(string)//"_grid_corner2.nc", rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        endif
        call ESMF_FieldDestroy(field(1),rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    if (isPresent) then
      ...
      field(1) = ESMF_FieldCreate(grid, array, staggerloc=ESMF_STAGGERLOC_CENTER, name="mask", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return

!      call ESMF_ArrayWrite(array, trim(string)//"_grid_mask.nc", rc=rc)
      if (multitile) then
        fieldR(1) = ESMF_FieldCreate(grid, ESMF_TYPEKIND_R8, name="mask", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        call ESMF_FieldGet(field(1) , farrayPtr=iptr, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        call ESMF_FieldGet(fieldR(1), farrayPtr=rptr, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        rptr = iptr
        call ESMFIO_Write(IOComp, trim(string)//"_grid_mask.nc", fieldR, filePath='./', rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
        call ESMF_FieldDestroy(fieldR(1),rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      else
        call ESMF_FieldWrite(field(1), trim(string)//"_grid_mask.nc", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      endif
      call ESMF_FieldDestroy(field(1),rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
    ...
    if (isPresent) then
      ...
      field(1) = ESMF_FieldCreate(grid, array, staggerloc=ESMF_STAGGERLOC_CENTER, name="area", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
...
!      call ESMF_ArrayWrite(array, trim(string)//"_grid_area.nc", rc=rc)
      if (multitile) then
        ...
        call ESMF_FieldWrite(field(1), trim(string)//"_grid_area.nc", rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
      endif
      call ESMF_FieldDestroy(field(1),rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
